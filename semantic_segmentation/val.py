...
from src.utils import multi_val_fn,op_flops_funs
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/data/sdu02_peach/Paddle/EMRT_256x256_160k_potsdam_resnet50_pretrain_os32/best_model.pdparams"
        # default="/data/sdu02_peach/Paddle/EMRT_256x256_120k_vaihingen_resnet50_pretrain_os32/best_model.pdparams"
        # default="/data/sdu02_peach/Paddle/EMRT_256x256_160k_loveda_resnet50_pretrain_os32/best_model.pdparams"
    )
    parser.add_argument(
        "--multi_scales",
        type=bool,
...
if __name__ == '__main__':
    ...
    logger.info(
        "[EVAL] Images: {} mIoU: {:.4f} Acc: {:.4f} Kappa: {:.4f}  mF1: {:.4f}".format(len(dataset_val), miou, acc,
                                                                                       kappa, mf1))
    logger.info("[EVAL] Class IoU: " + str(np.round(class_iou, 4)))
    logger.info("[EVAL] Class Acc: " + str(np.round(class_precision, 4)))
    logger.info("[EVAL] Class F1-Score: " + str(np.round(class_f1, 4)))

    # Calculate flops.
    if local_rank == 0:
        # _, c, h, w = img.shape
        _ = paddle.flops(model, [1, 3, 256, 256],
            custom_ops={paddle.nn.SyncBatchNorm: op_flops_funs.count_syncbn}, print_detail=False)
    ...
    print(f'Non-trainable params: {NonTrainable_params /1000000.0}')