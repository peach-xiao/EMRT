...
def calculate_area(pred, label, num_classes, ignore_index=255):
    ...
    if not pred.shape == label.shape:
        raise ValueError('Shape of `pred` and `label should be equal, '
                         'but there are pred{} and label{}.'.format(pred.shape, label.shape))
...
def accuracy(intersect_area, pred_area, label_area):
    """
    Calculate accuracy
    ...
    Returns:
        ...
        mean_acc (float): mean accuracy.  平均准确率。
    """
    ...
    # OA = np.sum(intersect_area) / np.sum(label_area)
    # print("mean_acc,OA",mean_acc,OA)
   ...
   # intersect_area = np.diag(hist), pred_area =  hist.sum(axis=1)  , label_area = hist.sum(axis=0)
...
def kappa(intersect_area, pred_area, label_area):
    ...
    label_area = label_area.numpy()
    total_area = np.sum(label_area)
    po = np.sum(intersect_area) / total_area  # po = OA
