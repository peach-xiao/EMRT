...
class EMRT_CSwin(nn.Layer):
    def __init__(self, config):
        super(EMRT_CSwin, self).__init__()
        ...
        if self.backbone == "cswintransformer":
        ...
        if self.backbone == "cswintransformer":
            ...
            for i in range(self.num_layers):
                self.num_features.append(
                    int(config.MODEL.TRANS.EMBED_DIM * 2 ** i))  # EMBED_DIM 64：[64, 128, 256, 512]

            self.layer_norms = nn.LayerList()
            for idx in self.backbone_out_indices:  # [0, 1, 2, 3]
                self.layer_norms.append(nn.LayerNorm(self.num_features[idx]))  # nn.LayerNorm[64, 128, 256, 512])

        if self.backbone == 'vit':  #OUT_INDICES: [9, 14, 19, 23]
            self.backbone_out_indices = [0, 1, 2, 3]
            self.num_features = [1024, 1024, 1024, 1024]
            self.layer_norms = nn.LayerList()
            for idx in self.backbone_out_indices:  # [0, 1, 2, 3]
    ...
    def forward(self, inputs):
        ...
        c1, c2, c3, c4 = feats
        # cswin:[8, 64, 56, 56] [8, 128, 28, 28] [8, 256, 14, 14] [8, 512, 7, 7]
        # vit：[8, 1024, 16, 16],[8, 1024, 16, 16],[8, 1024, 16, 16],[8, 1024, 16, 16]
        ...
        x1_index = x_fea[1].shape[-1] * x_fea[1].shape[-2]  # 14 * 14 = 196
        x2_index = x_fea[2].shape[-1] * x_fea[2].shape[-2]  # 7 * 7 = 49
        ...
        if self.backbone == 'vit':
            x_fpn = F.interpolate(x_fpn, size=x_context.shape[2:], mode='bilinear', align_corners=True)
