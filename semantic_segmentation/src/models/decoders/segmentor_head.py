...
class MaskTransformer(nn.Layer):
    ...
    def __init__(self, config):
        ...
        weight_attr_patch = paddle.ParamAttr(initializer=nn.initializer.Normal(std=hidden_size ** -0.5))
        ...
        weight_attr_class = paddle.ParamAttr(initializer=nn.initializer.Normal(std=hidden_size ** -0.5))
    ...
    def forward(self, x):
        ...
        x = self.proj_decoder(x) #shape=[4, 1024, 1024]
        mask_tokens = self.mask_tokens.expand((x.shape[0], -1, -1))  # 4 ,6(num_class),1024
        x = paddle.concat([x, mask_tokens], axis=1)  #shape=[4, 1030, 1024] 嵌入类别概率
        for layer in self.layers:  # ViT中的 attention + mlp
            x, _ = layer(x)
        x = self.decoder_norm(x)

        patches, masks = x[:, :-self.cls_num], x[:, -self.cls_num:]
        # print("patches",patches) #shape=[4, 1024, 1024]
        # print("masks",masks) #shape=[4, 6, 1024]
        patches = self.proj_patch(patches)  # shape=[4, 1024, 1024] 全连接层 1024 -- 1024
        masks = self.proj_class(masks)  # #shape=[4, 6, 1024] 全连接层 1024 -- 1024
        patches = patches / paddle.norm(patches, axis=-1, keepdim=True)#shape=[4, 1024, 1024]
        masks = masks / paddle.norm(masks, axis=-1, keepdim=True) #shape=[4, 6, 1024]
        masks = patches @ masks.transpose((0, 2, 1))
        masks = self.mask_norm(masks)# shape=[4, 1024, 6]

        # [b, (h w), n] -> [b, n, h, w]
        masks = masks.reshape((masks.shape[0], H, W, masks.shape[-1])) #shape = [4,32,32,6]
        masks = masks.transpose((0, 3, 1, 2)) #shape=[4, 6, 32, 32]
        return masks
...
class LinearDecoder(nn.Layer):
    ...
    def __init__(self, config):
        super().__init__()
        self.feature_size = (config.DATA.CROP_SIZE[0] // config.MODEL.TRANS.PATCH_SIZE,
                             config.DATA.CROP_SIZE[1] // config.MODEL.TRANS.PATCH_SIZE)  # 512 // 16
        self.head = nn.Linear(config.MODEL.TRANS.HIDDEN_SIZE, config.DATA.NUM_CLASSES)  # 1024 --> 6
