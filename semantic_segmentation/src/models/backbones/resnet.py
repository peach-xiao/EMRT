...
class BasicBlockV1b(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None,
        ...
        self.conv2 = nn.Conv2D(planes, planes, 3, 1, previous_dilation,
                               dilation=previous_dilation, bias_attr=False) #padding = previous_dilation
...
class ResNetV1(nn.Layer):
    ...
    def __init__(self, block, layers, config, num_classes=1000, deep_stem=False,
        ...
        scale = config.MODEL.BACKBONE_SCALE # default : 1.0
        if output_stride == 32:
            dilations = [1, 1]
            strides = [2, 2]
        elif output_stride == 16:  #default = 16
            dilations = [1, 2]
            strides = [2, 1]
        ...
        if deep_stem: #是否替换为3X3的 backbone
            # resnet vc
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, norm_layer=nn.BatchNorm2D,
        ...
        layers = []
        if not multi_grid: #multi_grid = false
            if dilation in (1, 2):
...
def resnet50c(config, norm_layer=nn.BatchNorm2D):
    ...
    """
    num_block = [3, 4, 6, 3] #  18=[2, 2, 2, 2]  34,50=[3, 4, 6, 3], 101=[3, 4, 23, 3],152= [3, 8, 36, 3]
    #resnet50及以上channel = 256,512,1024,2048, 以下的channel[64, 128, 256, 512]
...
def get_segmentation_backbone(backbone, config, norm_layer=paddle.nn.BatchNorm2D):
    ...
    """
    model = resnet50c(config, norm_layer) #7x7conv --> 3x3conv
    load_backbone_pretrained(model, backbone, config) #backbone -- resnet50c
    return model
