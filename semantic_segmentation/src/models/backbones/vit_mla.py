...
class Embeddings(nn.Layer):
    ...
    def __init__(self, config, in_channels=3):
        ...
        if config.MODEL.TRANS.PATCH_GRID is not None:  # false
            self.hybrid = True
            grid_size = config.MODEL.TRANS.PATCH_GRID #None
            patch_size = (image_size[0] // 16 // grid_size, image_size[1] // 16 // grid_size)
            n_patches = (image_size[0] // 16) * (image_size[1] // 16)
        else:
            self.hybrid = False
            patch_size = config.MODEL.TRANS.PATCH_SIZE # 16
            n_patches = (image_size[0] // patch_size) * (image_size[1] // patch_size)

        self.patch_embeddings = nn.Conv2D(in_channels=in_channels,
                                          out_channels=config.MODEL.TRANS.HIDDEN_SIZE, #1024
    ...
    def forward(self, x):
        # print("cls_tokens ", self.cls_token ) #shape=[1, 1, 1024]
        # print("cls_tokens[0] ", self.cls_token[0]) #shape=[1, 1024],
        cls_tokens = self.cls_token[0].expand((x.shape[0], -1, -1))#shape=[4, 1, 1024]
        ...
        # print("concat cls token: ",x) #shape=[4, 1025, 1024],
        # print("self.position_embeddings", self.position_embeddings) #shape=[1, 1025, 1024]
        # print("self.position_embeddings[0]",self.position_embeddings[0]) #shape=[1025, 1024]
        embeddings = x + self.position_embeddings[0]  # tensor broadcast
        # print("embeddings0", embeddings)  #shape=[4, 1025, 1024],
        embeddings = embeddings[:, 1:]  # For SETR
        # print("embeddings",embeddings) #shape=[4, 1024, 1024]
...
class Attention(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.num_heads = config.MODEL.TRANS.NUM_HEADS #16
        self.attn_head_size = int(config.MODEL.TRANS.HIDDEN_SIZE / self.num_heads) # 1024 // 16
...
class Encoder(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.out_idx_list = tuple(range(config.MODEL.TRANS.NUM_LAYERS)) # 24
...
class Transformer(nn.Layer):
    ...
    def forward(self, x):
        ...
        # print("embedding_out",embedding_out)# shape=[4, 1024, 1024]
...
class Conv_MLA(nn.Layer):
    ...
    def forward(self, res2, res3, res4, res5):
        ...
        mla_p5_1x1 = self.mla_p5_1x1(res5)  # 1024 -> 256
        mla_p4_1x1 = self.mla_p4_1x1(res4)  # 1024 -> 256
        mla_p3_1x1 = self.mla_p3_1x1(res3)  # 1024 -> 256
        mla_p2_1x1 = self.mla_p2_1x1(res2)  # 1024 -> 256
...
class ViT_MLA(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.transformer = Transformer(config)  # HIDDEN_SIZE = 1024,  MLA.MLA_CHANNELS = 256
        self.mla = Conv_MLA(in_channels=config.MODEL.TRANS.HIDDEN_SIZE, mla_channels=config.MODEL.MLA.MLA_CHANNELS)
        self.mla_index = config.MODEL.ENCODER.OUT_INDICES  # Large: [5, 11, 17, 23]
        ...
        if config.MODEL.PRETRAINED is not None:
            print('Load pretrained backbone from local path: {}!'.format(config.MODEL.PRETRAINED))
            load_pretrained_model(self, config.MODEL.PRETRAINED)
            # 是否将Conv_MLA模块的学习率系数设置为config.TRAIN.DECODER_LR_COEF(默认为1)
    ...
    def forward(self, x):
        ...
        c6 = self.norm_0(outs[self.mla_index[0]]) # Large: [5, 11, 17, 23]
