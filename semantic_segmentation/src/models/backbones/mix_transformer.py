...
class PatchEmbed(nn.Layer):
    """
    use a conv layer to implement PatchEmbed.        使用conv层来实现PatchEmbed。
    odd kernel size perform overlap patch embedding  奇数内核大小执行重叠补丁嵌入
    even kernel size perform non-overlap patch embedding    偶数内核大小执行非重叠补丁嵌入
    Args:
    ...
    def __init__(self,
                 in_channels=3,
                 embed_dims=768,
                 kernel_size=16,  # [7,3,3,3]
                 stride=16,  # [4, 2, 2, 2]
                 padding=0,
                 pad_to_patch_size=True):  # False
    ...
    def forward(self, x):
        ...
        if self.pad_to_patch_size:  # False
...
class EfficientAttention(nn.Layer):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:  # True
            x_ = nlc_to_nchw(x, H, W)
            x_ = self.sr(x_) #通过conv裁剪patch
            x_ = nchw_to_nlc(x_)
            x_ = self.norm(x_)
...
class TransformerEncoderLayer(nn.Layer):
    ...
    def __init__(self,
                 ...
                 num_heads,  # [1,2,5,8]
                 feedforward_channels,  # mlp_ratio * embed_dims, mlp_ratio = 4
                 drop_rate=0.,
                 attn_drop_rate=0.,
                 drop_path_rate=0.,
                 qkv_bias=True,
...
class MixVisionTransformer(nn.Layer):
    ...
    def __init__(self,
                 ...
                 embed_dims=64,
                 num_stages=4,
                 num_layers=[3, 4, 6, 3],  # [3, 6, 40, 3]
                 num_heads=[1, 2, 4, 8],  # [1, 2, 5, 8]
                 patch_sizes=[7, 3, 3, 3],
                 strides=[4, 2, 2, 2],
                 sr_ratios=[8, 4, 2, 1],
                 out_indices=(0, 1, 2, 3),
        ...
        for i, num_layer in enumerate(num_layers):  # [3, 6, 40, 3]
            embed_dims_i = embed_dims * num_heads[i]  # 64 * [1,2,5,8]
            patch_embed = PatchEmbed(in_channels=in_channels,  # 3, 64 ,128, 320
                                     embed_dims=embed_dims_i,  # 64 ,128, 320, 512
            ...
            layer = nn.LayerList([
                TransformerEncoderLayer(
                    ...
                    num_heads=num_heads[i],  # [1,2,5,8]
                    feedforward_channels=mlp_ratio * embed_dims_i,  # 4*C
                    drop_rate=drop_rate,
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x, H, W = layer[0](x), layer[0].DH, layer[0].DW  # layer[0] = patch_embed,
            for block in layer[1]:  # layer[1] = TransformerEncoderLayer
                x = block(x, H, W)
            x = layer[2](x)  # layer = LayerNorm()
            x = nlc_to_nchw(x, H, W)  # B, L(H*w), C -> B,C,H,W
