...
from src.models.backbones.paddleseg_utils import layers
from src.models.backbones.paddleseg_utils import utils
...
class HRNet(nn.Layer):
    ...
    def forward(self, x):
        ...
        conv2 = self.conv_layer1_2(conv1) #stage1的基本结构

        la1 = self.la1(conv2) # 过渡结构，BottleNeck

        tr1 = self.tr1([la1]) #TransitionLayer是用来生成一个下采样两倍分支的输入feature map的
        st2 = self.st2(tr1) #stage2的过渡结构，是一个FuseLayer和一个TransitionLayer的叠加
        # FuseLayer是用来进行不同分支的信息交互的，
        tr2 = self.tr2(st2)
        st3 = self.st3(tr2)# stage2的过渡结构，是一个FuseLayer和一个TransitionLayer的叠加

        tr3 = self.tr3(st3)
        st4 = self.st4(tr3)# stage4的过渡结构，是一个FuseLayer
