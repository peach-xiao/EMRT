...
class Embeddings(nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.hybrid:
            ...
            pass
        # print("x1",x) #shape=[4, 3, 512, 512]
        x = self.patch_embeddings(x) # shape=[4, 1024, 32, 32] 切分成N块P * P大小的图像块 N = HW / P*P
        x = x.flatten(2) #shape=[4, 1024, 1024] 从2维开始展开，将后面的维度转化为一维.也就是说，只保留dim之前的维度，其他维度的数据全都挤在dim这一维。
        x = x.transpose([0, 2, 1]) #shape=[4, 1024, 1024] 后面的1024是指的通道数目
        x = paddle.concat((cls_tokens, x), axis=1) #shape=[4, 1025, 1024],
        embeddings = x + self.position_embeddings[0] # tensor broadcast
        # print("shape",self.position_embeddings.shape,self.position_embeddings[0].shape) #[1, 1025, 1024] [1025, 1024]
        # shape=[4, 1025, 1024] + #shape=[1025, 1024] -> shape=[4, 1025, 1024]
        if not self.keep_cls_token: #针对于SETR, true, self.keep_cls_token = false
            embeddings = embeddings[:, 1:]  # For SETR  #shape=[4, 1024, 1024]
        embeddings = self.dropout(embeddings)
        # print("embeddings", embeddings) #shape=[4, 1025, 1024] or #SETR shape=[4, 1024, 1024]
        return embeddings
...
class Encoder(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.LayerList([EncoderLayer(config) for _ in range(config.MODEL.TRANS.NUM_LAYERS)]) #large 24
...
class VisualTransformer(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.out_indices = config.MODEL.ENCODER.OUT_INDICES #[9, 14, 19, 23] 和这里和setr-mla有所不同
