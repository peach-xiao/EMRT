...
class CSwinBlock(nn.Layer):
    ...
    def __init__(self,
        ...
        self.split_size = split_size  # [1, 2, 7, 7]
        ...
        for _ in range(num_branches): # 2 and last = 1
            attn = LePEAttention(dim=dim // num_branches,
                                 h_split=splits[0],
                                 w_split=splits[1],
                                 num_heads=num_heads // num_branches, ## [2, 4, 8, 16] // 2
                                 qk_scale=qk_scale,
...
class CSwinStage(nn.Layer):
    ...
    def __init__(self,
                 dim,
                 input_resolution,
                 depth,  # [1, 2, 21, 1] #当前阶段swin_block的数量
                 num_heads,  # [2, 4, 8, 16]
                 split_size,  # [1, 2, 7, 7]
                 mlp_ratio=4.,
                 qkv_bias=True,
    ...
    def forward(self, x):
        for block in self.blocks:
            x = block(x)
        x_down = self.merge(x)  # 通过一个conv3x3, 步长2, 实现下采样2倍
        return x, x_down
...
class CSwinTransformer(nn.Layer):
    ...
    def __init__(self, config):
        ...
        image_size = config.DATA.CROP_SIZE  # 512 512
        patch_stride = config.MODEL.TRANS.PATCH_SIZE  # 4
        in_channels = config.MODEL.TRANS.IN_CHANNELS
        num_classes = config.DATA.NUM_CLASSES
        embed_dim = config.MODEL.TRANS.EMBED_DIM
        depths = config.MODEL.TRANS.STAGE_DEPTHS  # [1, 2, 21, 1]
        splits = config.MODEL.TRANS.SPLIT_SIZES  # [1, 2, 7, 7]
        num_heads = config.MODEL.TRANS.NUM_HEADS  # [2, 4, 8, 16]
        mlp_ratio = config.MODEL.TRANS.MLP_RATIO  # 4
        qkv_bias = config.MODEL.TRANS.QKV_BIAS  # True
        qk_scale = config.MODEL.TRANS.QK_SCALE  # None
        dropout = config.MODEL.DROPOUT
        attention_dropout = config.MODEL.ATTENTION_DROPOUT
        droppath = config.MODEL.DROP_PATH  # 0.2
        ...
        depth_decay = [x.item() for x in paddle.linspace(0, droppath, sum(depths))]  # 生成sum(depths)个数，大小在0 - 0.2
        dim = embed_dim
        ...
        for stage_idx in range(num_stages):  # 4
            stage = CSwinStage(dim=dim,
                               ...
                               depth=depths[stage_idx],  # [1, 2, 21, 1]
                               num_heads=num_heads[stage_idx],  # [2, 4, 8, 16]
                               split_size=splits[stage_idx],  # [1, 2, 7, 7]
                               mlp_ratio=mlp_ratio,
                               qkv_bias=qkv_bias,
                               qk_scale=qk_scale,
                               dropout=dropout,
                               ...
                               droppath=depth_decay[
                                        sum(depths[:stage_idx]):sum(depths[:stage_idx + 1])],  # depths[1, 2, 21, 1]
                               last_stage=stage_idx == num_stages - 1)
            self.stages.append(stage)
            if stage_idx != num_stages - 1:  # 不为最后一层
                dim = dim * 2
                resolution = resolution // 2

        # 自行实现，加载预训练模型，默认是没有加载的
