...
import math
from .resnet_utils.save_load import load_dygraph_pretrain, load_dygraph_pretrain_from_url

# ResNeX is copy from paddleclas
    ...
    def forward(self, inputs):
        with paddle.static.amp.fp16_guard():
            ...
            for block in self.block_list:
                y = block(y)
                resnext_out.append(y)

            # y = self.pool2d_avg(y)
            # y = paddle.reshape(y, shape=[-1, self.pool2d_avg_channels])
            # y = self.out(y)
            # return y
            # c1, c2, c3, c4 = resnext_out[2], resnext_out[6], resnext_out[12], resnext_out[15]
            return resnext_out[2], resnext_out[6], resnext_out[12], resnext_out[15]
...
def ResNeXt50_32x4d(pretrained=False, use_ssld=False, **kwargs):
    print("using backbone: ResNeXt50_32x4d")
...
def ResNeXt50_64x4d(pretrained=False, use_ssld=False, **kwargs):
    print("using backbone: ResNeXt50_64x4d")
