...
class TransformerDecoder(nn.Layer):
    ...
    def __init__(self, input_size=512 // 4, embed_dim=768, depth=4, nclass=12,
        ...
        self.cls_embed0 = paddle.create_parameter(shape=[1, nclass, embed_dim*(2 ** (depth - 1))], dtype='float32',
                                                 default_initializer=nn.initializer.Constant(0.0))
        ...
        for i_layer in range(depth):
            ...
            # if i_layer ==0 : #先计算一个queary, 然后在上采样2倍数
            # layer_up = nn.LayerList([
            #     Block_Decoder(
            #         dim=embed_dim * (2 ** (depth - 1 - i_layer)), #512,256,128,64
            #         num_heads=num_heads, mlp_ratio=mlp_ratio, feat_HxW=decoder_feat_HxW * (4 ** i_layer),
            #         qkv_bias=qkv_bias,
            #         qk_scale=qk_scale,
            #         drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i_layer], norm_layer=norm_layer) ])
            # else:
            #     layer_up = BasicLayer_up(
            #         dim=int(embed_dim * 2 ** (depth - 1 - i_layer)), #256, 128, 64
            #         input_resolution=(input_size // (2 ** (depth - 1 - i_layer)),input_size // (2 ** (depth - 1 - i_layer))),
            #         nclass = nclass,
            #         num_heads= num_heads,
            #         mlp_ratio= mlp_ratio,
            #         qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.blocks_decoder = nn.LayerList([
            Block_Decoder(
                dim=embed_dim * (2 ** (depth - 1 - i)),
                num_heads=num_heads, mlp_ratio=mlp_ratio, feat_HxW=decoder_feat_HxW * (4 ** i), qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
            for i in range(depth)])
    ...
    def forward_decoder(self, encoder_out):
        #encoder_out: shape=[8, 16384, 64],shape=[8, 4096, 128],shape=[8, 1024, 256],shape=[8, 256, 512]
        attns_list = []
        ...
        for idx, blk in enumerate(self.blocks_decoder): #0,1,2,3
            if idx == 0:
                query = self.cls_embed[idx].expand([B, -1, -1])  # shape=[8, 6, 512] 将可学习的类原型嵌入作为query, shape=[1, nclass, embed_dim]
            else:
                # print("idx11,query", idx, query.shape) #shape=[8, 6, 512],[8, 6, 256],[8, 6, 128]
                query = self.upquery[idx](query)
                # print("idx22,query", idx, query.shape) #shape=[8, 6, 256],[8, 6, 128],[8, 6, 64]
                query += self.cls_embed[idx].expand([B, -1, -1])

                # print("idx11,feat", idx, feat.shape) #shape=[8, 256, 512],[8, 1024, 256],[8, 4096, 128]
                feat = self.patchExpand[idx](feat) #上采样2倍，通道缩减2倍
                # print("idx22,feat", idx, feat.shape) #shape=[8, 1024, 256],[8, 4096, 128],[8, 16384, 64]
                feat = paddle.concat([feat, encoder_out[3 - idx]], -1) #融入特征
                # print("idx33,feat", idx, feat.shape) #shape=[8, 1024, 512],[8, 4096, 256],[8, 16384, 128]
                feat = self.concat_back_dim[idx](feat)  # 2*dim  -> dim
                # print("idx44,feat", idx, feat.shape) #shape=[8, 1024, 256],[8, 4096, 128],[8, 16384, 64]

            attn, query, feat = blk(query, feat)
            attns_list.append(attn)
            # print(idx,attn) #shape=[8, 6, 8, 256](全0),[8, 6, 8, 1024],[8, 6, 8, 4096],[8, 6, 8, 16384]

        return attns_list,feat  #feat [8, 16384, 64]
