...
class SETR(nn.Layer):
    ...
    def __init__(self, config):
        ...
        if config.MODEL.ENCODER.TYPE == "ViT_MLA":  # MLA
            self.encoder = ViT_MLA(config)
        elif config.MODEL.ENCODER.TYPE == "ViT":  # PUP和Naive
        ...
        # 'PUP_VisionTransformerUpHead' 'Naive_VisionTransformerUpHead' ,MLA -- 'VIT_MLAHead'

        if self.decoder_type == "VIT_MLAHead":  # MLA
            self.decoder = VIT_MLAHead(
                config.MODEL.MLA.MLA_CHANNELS,  # 256
                config.MODEL.MLA.MLAHEAD_CHANNELS,  # 128
                config.DATA.NUM_CLASSES,
                config.MODEL.MLA.MLAHEAD_ALIGN_CORNERS)  # false
            ...
            self.decoder = VisionTransformerUpHead(
                config.MODEL.PUP.INPUT_CHANNEL,  # 1024
                config.MODEL.PUP.NUM_CONV,  # 4, native 2
                config.MODEL.PUP.NUM_UPSAMPLE_LAYER,  # 4 , native 1
                config.MODEL.PUP.CONV3x3_CONV1x1,  # True ,native false
                config.MODEL.PUP.ALIGN_CORNERS,  # false
                config.DATA.NUM_CLASSES)

            if self.auxi_head == True:  # True
                self.aux_decoder2 = VisionTransformerUpHead(
                    config.MODEL.AUXPUP.INPUT_CHANNEL,  # 1024
                    config.MODEL.AUXPUP.NUM_CONV,  # 2
                    config.MODEL.AUXPUP.NUM_UPSAMPLE_LAYER,  # 2, native 1
                    config.MODEL.AUXPUP.CONV3x3_CONV1x1,  # True, native false
    ...
    def init__decoder_lr_coef(self, config):
        ...
        for sublayer in self.decoder.sublayers():
            ...
            if isinstance(sublayer, nn.Conv2D):
                ...
                sublayer.weight.optimize_attr['learning_rate'] = config.TRAIN.DECODER_LR_COEF  # 10.0
