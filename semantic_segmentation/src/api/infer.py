...
def slide_inference(model, imgs, crop_size, stride_size, num_classes):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # print("\n logits:", logits.shape) #shape=[6, 6, 512, 512],
            # print("\n logits[0]:", logits[0])#shape=[6, 512, 512],
...
def ss_inference(model,
    ...
    Args:
        ...
        is_slide (bool): whether to infer by sliding window. 是否通过滑动窗口推断。
        base_size (list): the size of short edge is resize to min(base_size) 
        when it is smaller than min(base_size)   当短边的尺寸小于min(base_size)时，将其调整为min(base_size)。
    ...
    Returns:
        ...
        h, w) is returned.
    """
    if not is_slide: # 默认使用滑动窗口推断，不执行以下操作
        ...
        #  rescale or padding
        if rescale_from_ori: #恢复到原来尺寸的大小，默认为fALSE
...
def ms_inference(model,
    ...
    对于每个尺度，首先通过滑动窗口测试产生重叠的分割结果。然后将分割结果调整为原始尺寸，接着进行softmax操作。
    最后，所有尺度的分割对数被平均化+argmax
